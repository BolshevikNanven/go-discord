...
## 消息顺序一致性
- ~~采用id串行化，同一用户发送消息始终在同一节点(单方顺序一致性)，同一群聊消息始终在同一节点(群内顺序一致性)~~
- 雪花id
...
### redis缓存
- 缓存收件箱
- 群聊消息使用读扩散，须缓存群聊消息记录，并使用分布式锁防止缓存击穿，并设置不同的ttl防止缓存雪崩

### 长连接使用epoll
- 使用epoll维护websocket链接读事件，相较于per conn per routine节省大量内存

### 限流
- websocket长连接采用令牌桶(time/rate)限制user恶意刷消息
- grpc接口采用google SRE 熔断器进行客户端自适应限流，p=max((requests-K*accepts)/(requests+1))

### 链路追踪
- 使用jaeger+OTLP链路追踪

### grpc连接池
- 使用连接池维护多个链接，突破http2并发上限(http2会复用底层TCP连接，虽避免了http1.1的应用层头阻塞，仍可能存在tcp头阻塞)
